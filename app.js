...
function SvgIcon(props) {
  var icons = {
    ...
    'zap': html`<svg width=${props.size||18} height=${props.size||18} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>`,
  };
...
// ============================================================
function ExecutiveSummaryPage(props) {
  var projection = props.projection;
  var base = props.base;
  var bear = props.bear;
  var bull = props.bull;
  var mr = base.monthlyResults;
  var m1 = mr[0];
  var m12 = mr[mr.length - 1];

  // Core metrics
  var endingARR = m12.closingARR;
  var arrGrowthPct = ((endingARR - STARTING_ARR) / STARTING_ARR * 100);
  var endingMRR = m12.closingMRR;
  var m1MRR = m1.closingMRR;
  var endingCustomers = m12.totalCustomers;
  var customerGrowthPct = ((endingCustomers - EXISTING_CUSTOMERS) / EXISTING_CUSTOMERS * 100);
  var blendedMargin = m12.blendedGrossMargin;
  ...
  function getScenarioMetrics(scen) {
    var sm12 = scen.monthlyResults[scen.monthlyResults.length - 1];
    var sEndARR = sm12.closingARR;
    var sGrowth = (sEndARR - STARTING_ARR) / STARTING_ARR;
    var sMargin = sm12.blendedGrossMargin;
    var sFCF = sMargin - 0.55;
    return {
      endingARR: sEndARR, arrGrowth: sGrowth, margin: sMargin, fcfMargin: sFCF,
      val15x: sEndARR * 15, val20x: sEndARR * 20, val25x: sEndARR * 25,
      customers: sm12.totalCustomers, aiMix: sm12.aiMixPct,
    };
  }

  var bearM = getScenarioMetrics(bear);
  var baseM = getScenarioMetrics(base);
  var bullM = getScenarioMetrics(bull);

  // MRR decomposition for M12
  var expansionMRR_m12 = m12.expansionMRR;
  var contractionMRR_m12 = m12.contractionMRR;
  var churnedMRR_m12 = m12.churnedMRR;
  var newMRR_m12 = m12.newMRR;
  var aiMRR_m12 = m12.aiMRR;
  var baseMRR_m12 = m12.baseMRR;

  // Expansion vs churn ratio
  var netExpansion = expansionMRR_m12 - contractionMRR_m12 - churnedMRR_m12;

  // AI revenue trajectory
  var aiMRR_m1 = m1.aiMRR;
  var aiGrowthPct = aiMRR_m1 > 0 ? ((aiMRR_m12 - aiMRR_m1) / aiMRR_m1 * 100) : 0;

  // MRR per customer
  var mrrPerCustomer_start = STARTING_MRR / EXISTING_CUSTOMERS;
  var mrrPerCustomer_end = endingMRR / endingCustomers;
  var mrrPerCustomerGrowth = ((mrrPerCustomer_end - mrrPerCustomer_start) / mrrPerCustomer_start * 100);

  // Tier MRR at M12
  var tierMRR = getTotalMRRByTier(base, 12);
  var totalTierMRR = tierMRR.team + tierMRR.business + tierMRR.enterprise;
  var entPct = totalTierMRR > 0 ? (tierMRR.enterprise / totalTierMRR * 100) : 0;
  var busPct = totalTierMRR > 0 ? (tierMRR.business / totalTierMRR * 100) : 0;
  var teamPct = totalTierMRR > 0 ? (tierMRR.team / totalTierMRR * 100) : 0;

  // Monthly ARR growth rate (CAGR-ish)
  var monthlyGrowthRate = Math.pow(endingARR / STARTING_ARR, 1/12) - 1;

  function StatusDot(props2) {
    var color = props2.type === 'positive' ? '#20C933' : props2.type === 'warning' ? '#FCB400' : props2.type === 'risk' ? '#F82B60' : '#2D7FF9';
    return html`<span className="exec-dot" style=${{ background: color }}></span>`;
  }

  return html`
    <div className="exec-summary">
      <div className="exec-timestamp">12-Month Forward Projection from $478M ARR Base (Jan 2025 - Dec 2025)</div>

      <div className="exec-kpi-row">
        <div className="exec-kpi-card">
          <div className="exec-kpi-label">Projected ARR</div>
          <div className="exec-kpi-value">${fmtDollar(endingARR)}</div>
        ...
        <div className="exec-kpi-card">
          ...
          <div className="exec-kpi-value">${REPORTED_NRR}%</div>
          <div className="exec-kpi-delta positive">Top-decile SaaS</div>
        </div>
        <div className="exec-kpi-card">
          <div className="exec-kpi-label">Valuation Range (20x)</div>
          <div className="exec-kpi-value">${fmtDollar(baseM.val20x)}</div>
          <div className="exec-kpi-delta neutral">${fmtDollar(bearM.val20x)} - ${fmtDollar(bullM.val20x)}</div>
        </div>
      </div>
      ...
      <div className="exec-sections">
        ...
        <div className="exec-section">
          <div className="exec-section-header">
            <div className="exec-section-icon" style=${{ background: 'rgba(45,127,249,0.1)', color: '#2D7FF9' }}>\u2191</div>
            <div className="exec-section-title">Revenue Growth</div>
          </div>
          <ul className="exec-bullets">
            <li><${StatusDot} type="positive" /><span><strong>ARR grows ${arrGrowthPct.toFixed(0)}% ($478M \u2192 ${fmtDollar(endingARR)})</strong> \u2014 growth is entirely expansion-driven: seat additions and tier upgrades on existing accounts more than offset the revenue lost to logo churn, validating the PLG flywheel\u2019s compounding power</span></li>
            <li><${StatusDot} type="positive" /><span><strong>Monthly MRR compounds at ${(monthlyGrowthRate * 100).toFixed(1)}%/mo</strong> \u2014 $${(m1MRR/1e6).toFixed(1)}M (M1) \u2192 $${(endingMRR/1e6).toFixed(1)}M (M12), accelerating because cumulative cohort seat expansion compounds over time even as older cohorts churn</span></li>
            <li><${StatusDot} type="positive" /><span><strong>Net expansion MRR of ${fmtDollar(netExpansion)}/mo by M12</strong> \u2014 expansion (${fmtDollar(expansionMRR_m12)}) far exceeds contraction (${fmtDollar(contractionMRR_m12)}) + churn (${fmtDollar(churnedMRR_m12)}), confirming the PLG flywheel: surviving accounts expand seats 3\u20138%/mo, driving ARPU up even as logo count contracts</span></li>
            <li><${StatusDot} type="info" /><span><strong>Revenue per customer grows ${mrrPerCustomerGrowth.toFixed(0)}%</strong> ($${mrrPerCustomer_start.toFixed(0)} \u2192 $${mrrPerCustomer_end.toFixed(0)} MRR/customer) \u2014 the 170% NRR engine: Enterprise 8% monthly seat expansion + annual tier upgrades (10% Team\u2192Business, 5% Business\u2192Enterprise) compounds wallet share faster than logos churn</span></li>
          </ul>
        </div>

        <div className="exec-section">
          <div className="exec-section-header">
            <div className="exec-section-icon" style=${{ background: 'rgba(32,201,51,0.1)', color: '#20C933' }}>\u2605</div>
            <div className="exec-section-title">AI Monetization</div>
          </div>
          <ul className="exec-bullets">
            <li><${StatusDot} type="warning" /><span><strong>AI MRR tracks at ${fmtDollar(aiMRR_m12)}/mo (M12)</strong> \u2014 modest at ${(aiMixEnd * 100).toFixed(1)}% of total revenue because the $3.00/user ARPU on 30% adoption generates only ~$116K/mo; AI is currently a margin-dilutive add-on, not yet a growth driver. The opportunity is in pricing power, not volume</span></li>
            <li><${StatusDot} type="info" /><span><strong>AI margin drag is contained at under 1% revenue mix</strong> \u2014 at this scale, AI\u2019s lower 65% gross margin (vs. 85% base) has negligible impact on blended margin; the risk materializes only if AI mix exceeds 5\u201310% of total revenue</span></li>
            <li><${StatusDot} type="positive" /><span><strong>Bull case: AI ARPU at $5.00 + 60% adoption lifts ARR to ${fmtDollar(bullM.endingARR)}</strong> \u2014 if Airtable\u2019s AI automation features achieve viral adoption (similar to Notion AI trajectory), AI becomes the primary expansion lever; at 60% adoption + $5 ARPU, AI MRR could exceed $1M/mo</span></li>
          </ul>
        </div>

        <div className="exec-section">
          <div className="exec-section-header">
            <div className="exec-section-icon" style=${{ background: 'rgba(252,180,0,0.1)', color: '#FCB400' }}>\u25CE</div>
            <div className="exec-section-title">Margin & Unit Economics</div>
          </div>
          <ul className="exec-bullets">
        ...
        <div className="exec-section">
          <div className="exec-section-header">
            <div className="exec-section-icon" style=${{ background: 'rgba(139,70,255,0.1)', color: '#8B46FF' }}>\u25A0</div>
            <div className="exec-section-title">Customer Mix & Tier Dynamics</div>
          </div>
          <ul className="exec-bullets">
            <li><${StatusDot} type="positive" /><span><strong>Enterprise now ${entPct.toFixed(0)}% of base MRR</strong> (Business ${busPct.toFixed(0)}%, Team ${teamPct.toFixed(0)}%) \u2014 Enterprise\u2019s outsized share vs. its ~5% customer count reflects 50-seat averages at $85/seat ($4,250 MRR/account) vs. Team\u2019s 5 seats at $20 ($100 MRR/account), a 42x revenue density advantage</span></li>
            <li><${StatusDot} type="warning" /><span><strong>Logo count contracts to ${fmtNum(endingCustomers)} (${customerGrowthPct.toFixed(0)}%)</strong> \u2014 Team tier\u2019s 3.0% monthly churn bleeds ~3,500 logos/mo while PLG funnel adds only 600 new paid accounts/mo; however, this is revenue-accretive because churning accounts are low-ARPU Team ($100/mo) while surviving accounts expand rapidly</span></li>
            <li><${StatusDot} type="info" /><span><strong>Tier upgrade path is the hidden compounding engine</strong> \u2014 10% annual Team\u2192Business upgrade rate means ~2,800 accounts/year move from $100 \u2192 $540 avg MRR, adding ~$1.2M incremental monthly MRR; the 5% Business\u2192Enterprise path adds another ~$800K/mo</span></li>
          </ul>
        </div>

        <div className="exec-section">
          <div className="exec-section-header">
            <div className="exec-section-icon" style=${{ background: 'rgba(248,43,96,0.1)', color: '#F82B60' }}>\u26A0</div>
            <div className="exec-section-title">Scenario Range & Key Risks</div>
          </div>
          <ul className="exec-bullets">
            <li><${StatusDot} type="risk" /><span><strong>Bear case: ARR reaches only ${fmtDollar(bearM.endingARR)} (${(bearM.arrGrowth * 100).toFixed(0)}% growth)</strong> \u2014 triggered if conversion drops to 3% (loss of product-led momentum), seat expansion slows 40%, and AI cannibalizes margins without adding proportional revenue. Valuation impact: ${fmtDollar(bearM.val20x)} vs. ${fmtDollar(baseM.val20x)} base (\u2013${fmtDollar(baseM.val20x - bearM.val20x)})</span></li>
            <li><${StatusDot} type="positive" /><span><strong>Bull case: ARR reaches ${fmtDollar(bullM.endingARR)} (${(bullM.arrGrowth * 100).toFixed(0)}% growth)</strong> \u2014 achievable if AI adoption hits 60% at $5.00 ARPU, conversion improves to 5.5% (AI features as conversion catalyst), and Enterprise expansion accelerates to 10%/mo. Valuation: ${fmtDollar(bullM.val20x)} at 20x</span></li>
            <li><${StatusDot} type="warning" /><span><strong>Key swing factor: AI gross margin trajectory</strong> \u2014 current 65% AI margin is the single largest variable; a 10pp improvement (through inference cost negotiation to $0.0015/credit) would lift AI margin to 75%, preserving blended margin even as AI mix scales to 5\u201310%. This is the #1 metric to monitor quarterly</span></li>
          </ul>
        </div>

      </div>

      <div className="exec-scenario-bar">
        <div className="exec-scenario-bar-title">12-Month ARR Scenario Range</div>
        <div className="exec-range-visual">
          <div className="exec-range-track">
            <div className="exec-range-fill" style=${{ left: '0%', width: '100%' }}></div>
            <div className="exec-range-marker bear" style=${{ left: '0%' }}>
              <div className="exec-range-marker-label">Bear</div>
              <div className="exec-range-marker-value">${fmtDollar(bearM.endingARR)}</div>
            </div>
            <div className="exec-range-marker base" style=${{ left: '50%' }}>
              <div className="exec-range-marker-label">Base</div>
              <div className="exec-range-marker-value">${fmtDollar(baseM.endingARR)}</div>
            </div>
            <div className="exec-range-marker bull" style=${{ left: '100%' }}>
              <div className="exec-range-marker-label">Bull</div>
              <div className="exec-range-marker-value">${fmtDollar(bullM.endingARR)}</div>
            </div>
          </div>
        </div>
        <div className="exec-scenario-bar-sub">Valuation at 20x ARR: ${fmtDollar(bearM.val20x)} \u2014 ${fmtDollar(baseM.val20x)} \u2014 ${fmtDollar(bullM.val20x)}</div>
      </div>
...
function App() {
  ...
  var content = null;
  if (page === 'executive') content = html`<${ExecutiveSummaryPage} projection=${overviewProjection} base=${baseScenario} bear=${bearScenario} bull=${bullScenario} />`;
